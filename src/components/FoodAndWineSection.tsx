...
export default function FoodAndWineSection() {
  ...
  const visibleDishes = useMemo(() => {
    ...
    // *plus* the "greece" all-rounders, since they're served everywhere.
    // For "greece", show only the all-rounders.
    if (region === "greece") return dishes.filter(d => d.region === "greece");
    return dishes.filter(d => d.region === region || d.region === "greece");
  }, [region]);

  const visibleWineries = useMemo(
    () => wineries.filter(w => region === "greece" || w.region === region),
    [region]
  );
                  ...
                  >
                    ...
                    <div className="relative h-32 sm:h-36 bg-cream-200 overflow-hidden">
                      <PoiImage
                        src={dish.image}
                        alt={dish.name}
                        region={dish.region === "greece" ? "north" : dish.region}
                      />
